...
export function createGlobalStore<T, K extends string>(key: K, initialValue: T): () => (Record<"ready", boolean> & Record<`${K}`, T> & Record<`set${Capitalize<typeof key>}`, ((newValue: T) => void)> & Record<`remove${Capitalize<typeof key>}`, () => void> & Record<"init", ([key, setKey, removeKey]: [T, Dispatch<SetStateAction<T>>, () => void]) => void>) {
